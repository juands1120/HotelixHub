...
-- Tiempo de generación: 07-07-2025 a las 21:12:39
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_empleado` (IN `p_id_usuario` INT, IN `p_nombre` VARCHAR(100), IN `p_apellido` VARCHAR(100), IN `p_tipoDocumento` VARCHAR(100), IN `p_numeroDocumento` VARCHAR(100), IN `p_numeroTelefono` VARCHAR(100), IN `p_paisProcedencia` VARCHAR(100), IN `p_email` VARCHAR(100), IN `p_estado` VARCHAR(100), IN `p_direccion` VARCHAR(100))   BEGIN
    UPDATE usuarios
    SET nombre = p_nombre,
        apellido = p_apellido,
        ...
        paisProcedencia = p_paisProcedencia,
        email = p_email,
        estado = p_estado,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_empleado` (IN `p_id_usuario` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listar_roles_empleados` (IN `p_rol_nombre` VARCHAR(100))   BEGIN
    SELECT 
        u.id_usuario,
        u.usu_idrol,
        r.rol_nombre,
...
(1, '210', 'Sencilla', 2, 150000, 'Tv', 'Disponible', 'uploads/habitaciones/68685d47ce978_sencilla.jpg');
...
(3, 6, 1, '2025-07-07', '2025-07-09', 1, '[\"Spa\",\"Desayuno Buffet\"]', 415000.00, 'Pendiente', '2025-07-07 18:07:01');
...
(6, 2, 'Laura', 'Portillo', 'CC', '12456789', '1234567890', 'Colombia', 'lauportillo@gmail.com', '$2y$10$iKorjHsoDvBGF4H9ODLG.OaWhXWnjmaKulMidNk.3yK34r1qYUOs6', NULL, NULL, NULL, '');
